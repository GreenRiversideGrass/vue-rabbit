...
const open = async (row) => {
  ...
  // 设置 selectedValues value 的值
  selectedValues.value.value = [row.provinceCode, row.cityCode, row.countyCode]
  if (row.fullLocation) {
    // 设置 selectedValues 的 label
    let parts = row.fullLocation.split(" ")
    let current = selectedValues.value[0]
    parts.forEach((part) => {
      if (current) {
        current.label = part
...
const handleLocationChange = (val) => {
  ...
  // 拼接地址
  addressForm.value.fullLocation = cascaderRef.value
    .getCheckedNodes()[0]
    .pathLabels.join("")
}
...
<template>
  <el-dialog
    class="addAddress"
    ref="addFla"
    :title="addressForm.id ? '编辑收货地址' : '添加收货地址'"
    v-model="addflag"
    width="30%"
    center
  >
    ...
    >
      ...
      <el-form-item label="所在地区" prop="selectedValues">
        <el-cascader
          ref="cascaderRef"
          :options="options"
